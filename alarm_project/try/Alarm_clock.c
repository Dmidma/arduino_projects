...
{	
	...
	{	
		...
		// adding an else here will make the algrothime faster
		// since it won't test it each and every time
		// Setting the reminder on
		if (currentMode == 1 && reminder == 0)
		{	
			firstPoint = millis();
		...
		else if (currentMode == 1) // Checking if the user want to stop reminder
		{
			firstPoint = millis();
		...
		else if (currentMode == 2) // Checking if the user want to stop the alarm
		{
			firstPoint = millis();
	...
	if (tabModes[1].actLCD)
	{	
		if (dateTimeTab[1] == minRem && dateTimeTab[2] == houRem)
		{	
			firstPoint = millis();
	...
	if (tabModes[2].actLCD)
	{	
		if (dateTimeTab[1] == minAlarm && dateTimeTab[2] == houAlarm)
		{	
			firstPoint = millis();
...
/**
 * This function will initialize the Modes.
 * It will take x and y coor on the LCD and the char that will be printed
 * It will also take boolean value that serve for knowing if the Mode is *active or not.
 */
...
//This function will check the current value that is being changed 
//For Months we can have from 1 to 12
//	  Days 1 to 30 || 31 || 28 || 29
//	  Hours from 00 to 23
